function varargout = testgerilim(varargin)
% TESTGERILIM MATLAB code for testgerilim.fig
%      TESTGERILIM, by itself, creates a new TESTGERILIM or raises the existing
%      singleton*.
%
%      H = TESTGERILIM returns the handle to a new TESTGERILIM or the handle to
%      the existing singleton*.
%
%      TESTGERILIM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TESTGERILIM.M with the given input arguments.
%
%      TESTGERILIM('Property','Value',...) creates a new TESTGERILIM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before testgerilim_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to testgerilim_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help testgerilim

% Last Modified by GUIDE v2.5 07-Oct-2020 21:01:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @testgerilim_OpeningFcn, ...
                   'gui_OutputFcn',  @testgerilim_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before testgerilim is made visible.
function testgerilim_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to testgerilim (see VARARGIN)

% Choose default command line output for testgerilim
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes testgerilim wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = testgerilim_OutputFcn(hObject, eventdata, handles) 
...
switch answer
    case 'Devam et'
        testakim; %akým kalibrasyonu uygulamasý açýlýr.
    case 'Kapat'
        return;
end
